...
type Course = { id: string; title: string; content: string | null; fileUrl: string | null; subject: { name: string }; chapter: { name: string } | null };
...
function EvaluationManager({ subjects, chapters, evaluations, remove, update, message }: { subjects: Subject[]; chapters: Chapter[]; evaluations: Evaluation[]; create: (path: string, payload: object) => Promise<void>; remove: (path: string) => Promise<void>; update: (path: string, payload: object) => Promise<void>; message: string }) {
  ...
  // B38 : on lit `revisionSessionsCreated` pour avertir si 0 révision n'a été
  // planifiée (évaluation sans chapitre ou le jour même), au lieu du message
  // générique « Enregistré. ».
  function submit(event: FormEvent) {
    event.preventDefault();
    void (async () => {
      const response = await queuedFetch("/api/evaluations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, date, subjectId, chapterIds }) });
      if (response.status === 202) { setFeedback("Hors ligne : évaluation synchronisée dès la reconnexion."); return; }
      if (!response.ok) { setFeedback("Impossible d'enregistrer l'évaluation."); return; }
      const data = await response.json() as { revisionSessionsCreated?: number };
      setTitle(""); setDate(""); setChapterIds([]);
      setFeedback(data.revisionSessionsCreated === 0
        ? "Évaluation enregistrée, mais aucune révision n'a pu être planifiée (contrôle demain ou sans chapitre)."
        : `Évaluation enregistrée · ${data.revisionSessionsCreated} session(s) de révision créée(s).`);
    })();
  }
  function beginEdit(evaluation: Evaluation) { setEditingId(evaluation.id); setEditingTitle(evaluation.title); setEditingDate(toLocalDateInput(evaluation.date)); }
...
// B42 : formater une ISO en YYYY-MM-DD local (pas UTC) pour les <input type="date">.
...
function DataList({ title, empty, items }: { title: string; empty: string; items: Array<{ id: string; title: string; meta: string; action: () => void; secondaryAction?: () => void; secondaryLabel?: string; cancelAction?: () => void; editAction?: () => void; downloadHref?: string }> }) { return <section className="data-list"><div className="list-heading"><h2>{title}</h2><span>{items.length}</span></div>{items.length ? items.map((item) => <div className="data-item" key={item.id}><div><strong>{item.title}</strong><span>{item.meta}</span></div><div className="item-actions">{item.editAction && <button className="skip-button" onClick={item.editAction}>Modifier</button>}{item.secondaryAction && <button className="complete-button" onClick={item.secondaryAction}>{item.secondaryLabel}</button>}{item.cancelAction && <button className="skip-button" onClick={item.cancelAction}>Annuler</button>}{item.downloadHref && <a className="skip-button" href={item.downloadHref}><Download size={15} /> Fichier</a>}<button className="delete-button" onClick={() => { if (window.confirm(`Supprimer « ${item.title} » ? Cette action est définitive.`)) item.action(); }} aria-label={`Supprimer ${item.title}`}><Trash2 size={15} /></button></div></div>) : <p className="empty-state">{empty}</p>}</section>; }
