...
const jobSchema = z.object({ type: z.enum(["create_revision_plan", "sync_google_calendar", "process_course", "generate_study_sheet", "generate_flashcards", "generate_quiz", "update_mastery", "recalculate_workload"]), payload: z.record(z.string(), z.unknown()).default({}) });
...
export async function POST(request: Request) {
  ...
  const job = await enqueueJob(userId, parsed.data.type, parsed.data.payload);
