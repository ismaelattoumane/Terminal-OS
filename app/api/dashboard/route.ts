...
export async function GET() {
  ...
  const [todayRevisions, weekRevisions, dueHomework, upcomingEvaluations, chapters, grades, subjects] = await Promise.all([
    ...
    prisma.subject.findMany({ where: { userId: user.id }, include: { chapters: { select: { mastery: true } } }, orderBy: { name: "asc" } }),
  ]);
  ...
  return NextResponse.json({ today: { revisions: todayRevisions, homework: dueHomework, evaluations: upcomingEvaluations.slice(0, 3) }, week: { sessions: weekRevisions.length, plannedMinutes }, progression: { mastery: averageMastery, averageGrade: totalCoefficients ? Math.round((weightedGrades / totalCoefficients) * 100) / 100 : null }, workload, subjects, alerts: { overdueHomework: dueHomework.filter((homework) => homework.dueDate < today).length, weakChapters: chapters.filter((chapter) => chapter.mastery < 40).length } });
}