...
export async function GET() {
  ...
  const [todayRevisions, weekRevisions, dueHomework, upcomingEvaluations, chapters, grades, subjects] = await Promise.all([
    ...
    prisma.subject.findMany({ where: { userId: user.id }, include: { chapters: { select: { name: true, mastery: true } }, grades: { select: { grade: true, maxGrade: true, coefficient: true } } }, orderBy: { name: "asc" } }),
  ]);
  ...
  // Chapitre le plus faible toutes matières confondues : cible du panneau « Focus ».
